...
function median(x) {
	 ...
	 *  
	 * Returns
	 *  float: the median of the array x
	 */
	y = Array.copy(x);
	Array.sort(y);
	n = y.length;
...
function computeSBR(data, bandwidth) {
	/*
	 * Compute signal to base ratio (SBR): (f-f0)/f0
	 * 
	 * Args:
	 *  data (array): f values
	 *  bandwidth (float): size/length of the filter 
	 *  
	 * Returns:
	 *  array : (f-f0)/f0
	 */
...
function findOnEvents(score, baseline, threshold) {
	 ...
	 * The peaks need to be separated.
	 * 
	 * Args:
	 * 	score (array)
	 * 	tolerance (float)
...
function computeAllSignals(table_name, bandwidth, metric, prefilter, tolerance) {
	/*
	 * Extract signal from image for each ROI  and store them in a Table
	 * 
	 * Args:
	 *  table_name (str): title of the table
	 *  bandwidth (float): length of the filter
	 *  metric (array): sbr, signal, zscore
	 *  prefilter (boolean):  switch to enable adaptive prefiltering
	 *  tolerance (float): peak tolerance threshold
	 *  
	 * Returns:
	 *  Table with 5 columns (signal, sbr, zscrore, peks, on) per roi
	 */
	 ...
	 n = roiManager("count");
	 // for each ROI in the roi manager
	 for (i = 0; i < n; i++) {
	 	roiManager("select", i);
	 	name = Roi.getName();
...
function reportPeakEvents(tbl1, tbl2, interval, frames, unit) {
	 ...
	 *
	 * Args:
	 *  tbl1 (str): name of the signal table created by computeAllSignals
	 *  tbl2 (str): name of the peak table  (destination)
	 *  interval (float): frame interval
	 *  frames (int): number of frames
	 *  unit (str): time unit
	 *
	 * Returns:
	 *  Table tbl2 1 row per event with 5 columns (roi, event, frame, time, amplitude)
	 */
	selectWindow(tbl1);
...
function reportOnEvents(tbl1, tbl2, interval, frames, unit) {
	 ...
	 * 
	 * Returns:
	 *  Table with 1 row per on/off event and 5 columns (roi, event, frame, time, amplitude)
	 */
	K = 5;
	selectWindow(tbl1);
	columns = split(Table.headings,"\t");
...
function computePeakEventsStats(tbl1, tbl2, interval, frames, unit) {
	 ...
	 *
	 * Returns:
	 *  Table with statistics for each ROI with x,y,number of events, frequency and mean amplitude
	 */
	K = 5;
	selectWindow(tbl1);
	columns = split(Table.headings,"\t");
...
function segmentActiveROI() {
	/*
	 * Segment ROIs based on their brightness (unused)
	 *  
	 */
